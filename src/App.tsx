  ...
  return (
    <header className="header-corporate-glass sticky top-0 isolate z-40 w-full border-b border-white/15 bg-gradient-to-r from-[#8B0015] via-[#B9001B] to-[#D90829] px-6 py-2.5 text-white shadow-lg shadow-black/30 transition-shadow duration-500">
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-6 text-sm font-extrabold xl:flex">{links.map(([label, to]) => <Link key={to} to={to} className={menuLinkClass(to)}>{label}</Link>)}<div className="group relative"><button className="relative py-2 text-slate-100 transition hover:text-white after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-white after:transition-all group-hover:after:w-full">Bilgi Merkezi ▾</button><div className="information-dropdown invisible absolute right-0 top-full z-50 w-[540px] translate-y-1 rounded-b-2xl rounded-tl-2xl border border-slate-200 border-t-4 border-t-[#CD011E] bg-white/98 p-3 opacity-0 shadow-[0_18px_45px_rgba(30,10,15,.24)] backdrop-blur-xl transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"><div className="mb-3 flex items-center justify-between px-2 pt-1"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#CD011E] shadow-[0_0_10px_rgba(205,1,30,.65)]"/><p className="text-[10px] font-black tracking-[.18em] text-[#CD011E]">BİLGİ MERKEZİ</p></div><span className="text-[10px] font-bold text-slate-400">Keşfet →</span></div><div className="grid grid-cols-2 gap-3">{[['⌂','Gayrimenkul Rehberi','Alım, satım ve kiralama süreçleri'],['⚖','Gayrimenkul Hukuku','Tapu, sözleşme ve hukuki bilgiler'],['◉','Sektörden Haberler','Piyasa ve sektör gündemi'],['↗','Yatırım Analizleri','Bölge, fiyat ve getiri değerlendirmeleri']].map(([icon,title,description])=><Link key={title} to="/#akademi" className="information-dropdown-item group/item relative overflow-hidden rounded-xl border border-slate-200 bg-[#fffafa] p-4 transition hover:-translate-y-0.5 hover:border-[#CD011E]/45 hover:bg-white hover:shadow-lg"><div className="absolute right-[-8px] top-[-12px] text-4xl font-black text-[#CD011E]/[.06] transition group-hover/item:scale-110">{icon}</div><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#CD011E] text-base font-black text-white shadow-md shadow-[#CD011E]/25">{icon}</span><span className="mt-3 block text-xs font-black text-slate-900 transition group-hover/item:text-[#CD011E]">{title}</span><span className="mt-1 block text-[10px] font-medium leading-relaxed text-slate-500">{description}</span></Link>)}</div></div></div></nav>
