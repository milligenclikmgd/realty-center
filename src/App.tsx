...
function Header({ scrolled }: { scrolled: boolean }) {
  ...
  return (
    <header className={`header-corporate-glass sticky top-0 isolate z-40 w-full border-b border-white/10 text-white transition-all duration-500 ${scrolled ? 'bg-[#69000f]/94 py-1.5 px-5 lg:px-10 shadow-xl shadow-black/40 backdrop-blur-xl' : 'bg-[#850014]/91 py-2.5 px-6 lg:px-12 shadow-lg shadow-black/30 backdrop-blur-md'}`}>
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        <Link to="/" onClick={close} className={`relative -my-2 flex items-center rounded-r-2xl border border-white/15 bg-white/10 px-4 py-2 shadow-lg shadow-black/20 transition-transform hover:scale-[1.02] hover:bg-white/15 ${scrolled ? 'lg:-my-1.5' : ''}`}><img src="/rlogo.png" alt="Realty Center" className={`w-auto object-contain brightness-0 invert transition-all duration-500 ${scrolled ? 'h-11 lg:h-12' : 'h-12 lg:h-14'}`} /></Link>
