...
function Header({ language, setLanguage }: { language: StaticLanguage; setLanguage: (language: StaticLanguage) => void }) {
  ...
  const links = [
    [t.offices, '/ofislerimiz'], [t.agents, '/danismanlarimiz'],
    [t.listings, '/ilan-kategorileri'], [t.projects, '/projelerimiz']
  ];
  const trailingLinks = [[t.ai, '/ai-karar-asistani'], [t.contact, '/iletisim']];
  ...
  return (
    <header className="header-corporate-glass sticky top-0 isolate z-40 w-full border-b border-white/15 bg-gradient-to-r from-[#8B0015] via-[#B9001B] to-[#D90829] px-6 py-2.5 text-white shadow-lg shadow-black/30 transition-shadow duration-500">
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-6 text-sm font-extrabold xl:flex"><div className="group relative"><button className="relative py-2 text-slate-100 transition hover:text-white after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-white after:transition-all group-hover:after:w-full">Kurumsal ▾</button><div className="invisible absolute left-0 top-full z-50 w-72 translate-y-1 rounded-b-2xl rounded-tr-2xl border border-slate-200 border-t-4 border-t-[#CD011E] bg-white p-2 opacity-0 shadow-2xl transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">{[['Hakkımızda','/kurumsal/hakkimizda'],['Neden Realty Center?','/neden-realty-center'],['Yönetim Kurulumuz','/kurumsal/yonetim-kurulu'],['Referanslarımız','/kurumsal/referanslar']].map(([label,to])=><Link key={to} to={to} className="block rounded-xl px-3 py-3 text-xs font-black text-slate-800 transition hover:bg-[#fff1f3] hover:text-[#CD011E]">{label}<span className="mt-1 block text-[10px] font-medium text-slate-500">{label==='Hakkımızda'?'Vizyon, misyon ve değerler':label==='Yönetim Kurulumuz'?'Yönetim yapımız ve ekibimiz':label==='Referanslarımız'?'İş ortakları ve başarı hikâyeleri':'Güven, uzmanlık ve şeffaflık'}</span></Link>)}</div></div>{links.map(([label, to]) => <Link key={to} to={to} className={menuLinkClass(to)}>{label}</Link>)}<div className="group relative"><button className="relative py-2 text-slate-100 transition hover:text-white after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-white after:transition-all group-hover:after:w-full">BLOG ▾</button><div className="information-dropdown invisible absolute right-0 top-full z-50 w-80 translate-y-1 rounded-b-2xl rounded-tl-2xl border border-slate-200 border-t-4 border-t-[#CD011E] bg-white/98 p-3 opacity-0 shadow-[0_18px_45px_rgba(30,10,15,.24)] backdrop-blur-xl transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"><div className="mb-2 flex items-center gap-2 px-2 pt-1"><span className="h-1.5 w-1.5 rounded-full bg-[#CD011E]"/><p className="text-[10px] font-black tracking-[.18em] text-[#CD011E]">BLOG</p></div><div className="space-y-1">{[['⌂','Rehber','Gayrimenkul alım, satım ve kiralama','/blog/rehber'],['⚖','Hukuk','Tapu, sözleşme ve yasal bilgiler','/blog/hukuk'],['◉','Haberler','Piyasa ve sektörden gelişmeler','/blog/haberler'],['↗','Analizler','Yatırım, fiyat ve bölge analizi','/blog/analizler']].map(([icon,title,description,to])=><Link key={title} to={to} className="information-dropdown-item group/item flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-[#fff1f3]"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#CD011E] text-sm font-black text-white shadow-sm">{icon}</span><span className="min-w-0"><span className="block text-xs font-black text-slate-900 transition group-hover/item:text-[#CD011E]">{title}</span><span className="mt-0.5 block truncate text-[10px] font-medium text-slate-500">{description}</span></span></Link>)}</div></div></div>{trailingLinks.map(([label, to]) => <Link key={to} to={to} className={menuLinkClass(to)}>{label}</Link>)}</nav>
      ...
      {mobileOpen && <nav className="relative z-10 xl:hidden mt-3 grid grid-cols-2 gap-2 border-t border-white/10 pt-3 pb-2 text-sm font-black text-white">{links.map(([label, to]) => <Link key={to} onClick={close} to={to} className="rounded-xl bg-white/5 px-3 py-3 transition hover:bg-white/10">{label}</Link>)}<div className="col-span-2 rounded-xl bg-white/10 p-3"><p className="text-xs font-black text-white">BLOG</p><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] font-bold text-white/75"><Link onClick={close} to="/blog/rehber">Rehber</Link><Link onClick={close} to="/blog/hukuk">Hukuk</Link><Link onClick={close} to="/blog/haberler">Haberler</Link><Link onClick={close} to="/blog/analizler">Analizler</Link></div></div><Link onClick={close} to="/panel" className="col-span-2 rounded-xl border border-[#ef6577] bg-[#CD011E] px-3 py-3 text-center transition hover:brightness-110">Panele Git</Link></nav>}
    </header>
  );
}
