...
function MissionVisionPage() {
  ...
  return <main className="min-h-screen bg-slate-50 py-14"><div className="mx-auto max-w-6xl px-6"><p className="text-xs font-black tracking-[.2em] text-red-700">KURUMSAL YÖNÜMÜZ</p><h1 className="mt-3 text-4xl font-black text-slate-950">Misyonumuz ve Vizyonumuz</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">Realty Center’ın bugünkü hizmet anlayışını ve geleceğe dönük hedeflerini belirleyen kurumsal yaklaşımımız.</p><div className="mt-9 grid gap-6 md:grid-cols-2">{sections.map((section,index) => <article key={section.title} className="rounded-3xl bg-slate-950 p-8 text-white shadow-xl"><span className="text-4xl font-black text-red-600/70">0{index+1}</span><h2 className="mt-5 text-2xl font-black">{section.title}</h2><p className="mt-4 text-sm leading-7 text-slate-300">{section.text}</p></article>)}</div><section className="mt-8 rounded-3xl border border-slate-200 bg-white p-7 shadow-sm sm:p-10"><div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr]"><div><p className="text-[10px] font-black tracking-[.2em] text-red-700">ÖNCE GÜVEN</p><h2 className="mt-3 text-3xl font-black text-slate-900">Kalite Standartlarımız</h2><p className="mt-4 text-sm leading-7 text-slate-600">Her Realty Center ofisinde aynı güvenilir hizmet deneyimini oluşturmak için uyguladığımız temel standartlar.</p></div><div className="grid gap-3 sm:grid-cols-2">{standards.map((standard) => <div key={standard} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-xs font-black text-slate-700"><CheckCircle2 className="h-5 w-5 shrink-0 text-red-700"/>{standard}</div>)}</div></div></section></div></main>;
}
