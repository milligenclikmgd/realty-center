...
function BarterBankModuleV3() {
  ...
  const column = (title: string, type: typeof offer, setType: (type: typeof offer) => void, prefix: string) => <section className="border border-slate-200 bg-slate-50 p-4"><h3 className="text-base font-black text-slate-900">{title}</h3><p className="mt-1 text-xs text-slate-500">{title === 'TAKAS ÖNERİSİ' ? 'Vermek istediğiniz değer' : 'Almak istediğiniz değer'}</p><label className="mt-3 block text-[10px] font-black tracking-widest text-slate-600">DEĞER TÜRÜ<select value={type} onChange={(e) => setType(e.target.value as typeof offer)} className="mt-2 w-full border border-slate-200 bg-white p-2.5 text-sm"><option>Ev</option><option>Arsa</option><option>Otomobil</option><option>Diğer</option></select></label><div className="mt-3 grid gap-2">{fields(type).map((field) => <input key={field} required name={`${prefix}-${field}`} placeholder={field} className="border border-slate-200 bg-white px-3 py-2.5 text-sm"/>)}</div></section>;
  ...
  if (sent) return <section id="barter-bank" className="bg-slate-50 py-10"><div className="mx-auto max-w-5xl px-6"><div className="bg-[#CD011E] p-8 text-center text-white"><h2 className="text-2xl font-black">BARTERBANK® kaydınız alındı.</h2><p className="mt-2 text-sm text-white/85">Teklifiniz ve talebiniz yönetici paneline iletildi.</p></div></div></section>;
  return <section id="barter-bank" className="border-b border-slate-200 bg-slate-50 py-8 sm:py-10"><div className="mx-auto max-w-6xl px-6 lg:px-12"><div className="overflow-hidden border border-slate-200 bg-white shadow-lg"><header className="flex flex-col gap-4 bg-[#CD011E] p-5 text-white sm:flex-row sm:items-center sm:justify-between sm:px-7"><div><div className="flex flex-wrap items-baseline gap-x-3"><h2 className="text-2xl font-black tracking-[.08em] sm:text-3xl">BARTERBANK®</h2><span className="text-xs font-bold tracking-[.16em] text-white/60">EMLAK TAKAS SİSTEMİ</span></div></div><div className="flex items-center gap-3"><div aria-label="Fazlanı ver, eksiğini al" className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-full border border-white/45 bg-white/10 text-center text-[8px] font-black leading-tight tracking-wide"><span>FAZLANI VER</span><span className="my-0.5 h-px w-7 bg-white/60"></span><span>EKSİĞİNİ AL</span></div><button type="button" onClick={() => setOpen(!open)} aria-expanded={open} className="flex items-center gap-2 bg-white px-4 py-3 text-sm font-black text-[#CD011E] transition hover:bg-slate-100">{open ? 'Formu Kapat' : 'Takasa Başla'} <ArrowRight className={`h-4 w-4 transition-transform ${open ? 'rotate-90' : ''}`} /></button></div></header>{open && <form onSubmit={submit} className="border-t border-slate-200 p-5 sm:p-7"><div className="mb-4 flex items-center justify-between"><p className="text-sm font-bold text-slate-600">Tek formda hem takas önerinizi hem takas talebinizi girin.</p><button type="button" onClick={() => setOpen(false)} className="text-xs font-black text-red-700">KAPAT</button></div><div className="grid gap-4 lg:grid-cols-2">{column('TAKAS ÖNERİSİ', offer, setOffer, 'öneri')}{column('TAKAS TALEBİ', request, setRequest, 'talep')}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><select required name="Barter türü" className="border border-slate-200 px-4 py-2.5 text-sm"><option value="">Barter türünü seçin</option><option>Tam Barter</option><option>Kısmi Barter</option></select><input required name="Telefon" placeholder="Telefon numaranız" className="border border-slate-200 px-4 py-2.5 text-sm"/></div><label className="mt-4 flex items-start gap-2 text-[11px] leading-5 text-slate-500"><input required type="checkbox" className="mt-1 accent-red-700"/>KVKK ve <Link to="/kullanim-kosullari" className="font-black text-red-700">Site Kullanım Politikası</Link> metinlerini okudum ve onaylıyorum.</label><button className="mt-5 w-full bg-[#CD011E] px-5 py-3.5 text-sm font-black text-white">TAKAS ÖNERİSİ VE TALEBİNİ GÖNDER</button></form>}</div></div></section>;
}
