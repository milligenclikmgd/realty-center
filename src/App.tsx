...
const DEFAULT_HEADER_MENU: HeaderMenuItem[] = [
  ...
  headerItem('discover','Keşfet','/icerik/education','left',[
    ...
    headerItem('future','Geleceğim İçin','/icerik/future','left',[headerItem('build-my-team','Bana Takım Kur','/icerik/bana-takim-kur','left',[],'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&q=90&w=1400','İşinizi tek başına büyütmek yerine doğru rollerle güçlü bir ekip kurun. Realty Center; idari destek, alıcı müşteri yönetimi ve portföy uzmanlığı gibi kritik alanlarda sürdürülebilir bir takım yapısı oluşturmanız için yanınızdadır.')],'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&q=90&w=1400','Gayrimenkul sektöründe güçlü bir kariyer kurmanız için eğitim, mentorluk ve kurumsal marka desteğini bir araya getiriyoruz.'),
...
const getHeaderMenu = (): HeaderMenuItem[] => { try { const stored: HeaderMenuItem[] = (JSON.parse(localStorage.getItem('realty-center-header-menu') || 'null') || DEFAULT_HEADER_MENU).map((item: HeaderMenuItem)=>item.id==='library'?{...item,path:'/kutuphane'}:item); const discover = stored.find((item) => item.id === 'discover'); const future = discover?.children.find((item) => item.id === 'future'); if (future && !future.children.some((item) => item.id === 'build-my-team')) future.children.push(headerItem('build-my-team','Bana Takım Kur','/icerik/bana-takim-kur','left',[],'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&q=90&w=1400','İşinizi tek başına büyütmek yerine doğru rollerle güçlü bir ekip kurun. Realty Center; idari destek, alıcı müşteri yönetimi ve portföy uzmanlığı gibi kritik alanlarda sürdürülebilir bir takım yapısı oluşturmanız için yanınızdadır.')); const discoverIndex = stored.findIndex((item) => item.id === 'discover'); const videosIndex = stored.findIndex((item) => item.id === 'videos'); if (discoverIndex > videosIndex && videosIndex >= 0) { const reordered = [...stored]; const [discover] = reordered.splice(discoverIndex,1); reordered.splice(videosIndex,0,discover); return reordered; } return stored; } catch { return DEFAULT_HEADER_MENU; } };
