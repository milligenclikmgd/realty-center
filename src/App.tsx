...
function LibraryHomePage() {
  ...
  return <main className="min-h-[70vh] bg-slate-50 py-14"><div className="mx-auto max-w-7xl px-6"><p className="text-xs font-black tracking-[.24em] text-red-700">REALTY CENTER</p><h1 className="mt-3 text-4xl font-black text-slate-950">Realty Kütüphane</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">Sektörel gelişmeleri, mevzuatı, yapay zekâ destekli rehberleri ve yatırım analizlerini tek merkezden takip edin.</p><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{categories.map((category)=><Link key={category.id} to={`/kutuphane/${category.slug}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="h-2" style={{backgroundColor:category.color}}/><div className="p-6"><span className="text-[10px] font-black tracking-[.18em]" style={{color:category.color}}>REALTY KÜTÜPHANE</span><h2 className="mt-3 text-xl font-black text-slate-900">{category.title}</h2><p className="mt-3 min-h-16 text-xs leading-6 text-slate-600">{category.description}</p><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-xs font-black"><span className="text-slate-400">{category.articles.length} içerik</span><span style={{color:category.color}}>Bölümü aç →</span></div></div></Link>)}</div></div></main>;
}
...
function ListingCategoriesPage() {
  const navigate = useNavigate();
  const groups = [
    { title: 'SATILIK', icon: '🏠', transaction: 'Satılık', tone: 'border-red-100', head: 'text-red-700', cell: 'hover:border-red-300 hover:text-red-700', items: [['Konut','Ev','https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=70&w=500'],['Villa','Villa','https://images.unsplash.com/photo-1613490493576-7fde63acd811?auto=format&fit=crop&q=70&w=500'],['Arsa','Arsa','https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&q=70&w=500'],['Tümünü Gör','','https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&q=70&w=500']] },
    { title: 'KİRALIK', icon: '🔑', transaction: 'Kiralık', tone: 'border-blue-100', head: 'text-blue-800', cell: 'hover:border-blue-300 hover:text-blue-700', items: [['Konut','Ev','https://images.unsplash.com/photo-1600566753086-00f18fb6b3ea?auto=format&fit=crop&q=70&w=500'],['İşyeri','Dükkan','https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=70&w=500'],['Villa','Villa','https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&q=70&w=500'],['Tümünü Gör','','https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&q=70&w=500']] },
    { title: 'DEVREN', icon: '🔄', transaction: 'Devren Satılık', tone: 'border-violet-100', head: 'text-violet-700', cell: 'hover:border-violet-300 hover:text-violet-700', items: [['İşyeri','Dükkan','https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&q=70&w=500'],['Dükkan','Dükkan','https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&q=70&w=500'],['Ofis','Ofis','https://images.unsplash.com/photo-1497366811353-6870744d04b2?auto=format&fit=crop&q=70&w=500'],['Tümünü Gör','','https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&q=70&w=500']] },
    { title: 'PROJELER', icon: '🏗️', route: '/projelerimiz', tone: 'border-amber-100', head: 'text-amber-700', cell: 'hover:border-amber-300 hover:text-amber-700', items: [['Yeni Konut','','https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&q=70&w=500'],['Ticari Proje','','https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=70&w=500'],['Villa Projesi','','https://images.unsplash.com/photo-1600047509807-ba8f99d2cdde?auto=format&fit=crop&q=70&w=500'],['Tümünü Gör','','https://images.unsplash.com/photo-1503387762-592deb58ef4e?auto=format&fit=crop&q=70&w=500']] },
    { title: 'ARSA', icon: '🌳', transaction: 'Satılık', tone: 'border-emerald-100', head: 'text-emerald-700', cell: 'hover:border-emerald-300 hover:text-emerald-700', items: [['Konut','Arsa','https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&q=70&w=500'],['Ticari','Arsa','https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&q=70&w=500'],['Sanayi','Arsa','https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?auto=format&fit=crop&q=70&w=500'],['Tarla','Arsa','https://images.unsplash.com/photo-1500937386664-56d1dfef3854?auto=format&fit=crop&q=70&w=500']] },
    { title: 'TİCARİ', icon: '🏢', transaction: 'Satılık', tone: 'border-cyan-100', head: 'text-cyan-700', cell: 'hover:border-cyan-300 hover:text-cyan-700', items: [['Plaza','Ofis','https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=70&w=500'],['Ofis','Ofis','https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=70&w=500'],['Dükkan','Dükkan','https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&q=70&w=500'],['Tümünü Gör','','https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&q=70&w=500']] }
  ];
  ...
  const categoryEmoji = (label: string) => ({ 'Konut':'🏠', 'Villa':'🏡', 'Arsa':'🌳', 'İşyeri':'🏬', 'Dükkan':'🏪', 'Ofis':'💼', 'Plaza':'🏢', 'Yeni Konut':'🏗️', 'Ticari Proje':'🏙️', 'Villa Projesi':'🏘️', 'Ticari':'🏢', 'Sanayi':'🏭', 'Tarla':'🌾', 'Tümünü Gör':'✨' }[label] || '🏠');
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        <div className="flex flex-col gap-5 border-b border-slate-200 pb-8 sm:flex-row sm:items-end sm:justify-between"><div><span className="inline-flex rounded-full border border-red-200 bg-red-50 px-3.5 py-1.5 text-xs font-black tracking-widest text-red-700">PORTFÖY KATEGORİLERİ</span><h1 className="mt-4 text-3xl font-black text-slate-900 sm:text-4xl">Gayrimenkul kategorileri</h1><p className="mt-2 text-sm font-medium text-slate-600">Önce ana kategoriyi, ardından kare içindeki gayrimenkul türünü seçin.</p></div><Link to="/ilanlarimiz?all=1" className="inline-flex items-center justify-center rounded-xl bg-red-700 px-5 py-3 text-sm font-black text-white shadow-lg hover:bg-red-800">Tüm İlanları Gör <ArrowRight className="ml-2 h-4 w-4" /></Link></div>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{groups.map((group,index) => <section key={group.title} className={`rounded-3xl border bg-white p-3 shadow-[0_12px_30px_rgba(15,23,42,.06)] transition hover:-translate-y-1 hover:shadow-[0_18px_38px_rgba(15,23,42,.1)] ${group.tone}`}><header className="flex items-center justify-between px-2 py-2"><div className="flex items-center gap-2.5"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-50 text-xl shadow-inner">{group.icon}</span><div><h2 className={`text-sm font-black tracking-[.08em] ${group.head}`}>{group.title}</h2><p className="text-[9px] font-bold text-slate-400">Kategori seçin</p></div></div><span className="text-[10px] font-black text-slate-200">0{index+1}</span></header><div className="mt-2 grid aspect-[4/3] grid-cols-2 grid-rows-2 gap-2 rounded-2xl bg-slate-100/80 p-2">{group.items.map(([label,propertyType]) => <button key={`${group.title}-${label}`} onClick={() => openCategory(group, propertyType)} className={`group relative flex overflow-hidden rounded-xl border border-white bg-white p-3 text-left text-xs font-black text-slate-800 shadow-sm transition ${group.cell}`}><span className="absolute -right-1 -top-2 text-5xl opacity-[.12] grayscale transition duration-300 group-hover:scale-110 group-hover:opacity-25 group-hover:grayscale-0">{categoryEmoji(label)}</span><span className="relative mt-auto flex w-full items-end justify-between gap-1"><span><span className="mb-1 block text-xl">{categoryEmoji(label)}</span>{label}</span><ArrowRight className="h-3.5 w-3.5 shrink-0 text-slate-400 transition group-hover:translate-x-1 group-hover:text-current"/></span></button>)}</div></section>)}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="listing-detail-enter min-h-screen bg-white px-0 pb-6 pt-4 print:bg-white">
      <div className="mx-auto max-w-[1360px] px-3 sm:px-5 lg:px-6">
        ...
        <section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-black text-slate-900">Benzer İlanlar</h2><Link to={`/ilanlarimiz?propertyType=${encodeURIComponent(item.propertyType)}`} className="text-xs font-black text-red-700">Tümünü Gör →</Link></div>{similar.length ? <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">{similar.map((listing) => <ListingCard key={listing.id} item={listing}/>)}</div> : <div className="border border-slate-200 bg-white p-6 text-xs font-bold text-slate-500">Benzer ilan bulunamadı.</div>}</section>
      </div>
    </div>
  );
}
