  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{groups.map((group,index) => <section key={group.title} className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-xl"><div className="relative h-28 overflow-hidden"><img src={group.items[0][2]} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-r from-slate-950/85 via-slate-950/45 to-transparent"/><div className="absolute inset-x-5 bottom-4 flex items-end justify-between"><div><p className="text-[10px] font-black tracking-[.18em] text-white/70">{group.label}</p><h2 className="mt-1 text-lg font-black text-white">{group.title}</h2></div><span className="text-xs font-bold text-white/65">0{index+1}</span></div></div><div className="p-5"><p className="min-h-10 text-xs leading-5 text-slate-500">{group.description}</p><div className="mt-4 border-t border-slate-100">{group.items.map(([label,propertyType],itemIndex) => <button key={`${group.title}-${label}`} onClick={() => openCategory(group, propertyType)} className="flex w-full items-center justify-between gap-3 border-b border-slate-100 py-3 text-left last:border-0"><span className="text-sm font-bold text-slate-700 transition group-hover:text-slate-950">{label}</span><span className="flex items-center gap-2"><span className="hidden text-[10px] font-bold uppercase tracking-wide text-slate-400 sm:inline">{propertyType || 'Tüm seçenekler'}</span><ArrowRight className={`h-4 w-4 transition ${itemIndex === group.items.length-1 ? 'text-red-700' : 'text-slate-300'} group-hover:translate-x-1`}/></span></button>)}</div></div></section>)}</div>
      </div>
    </div>
  );
}
