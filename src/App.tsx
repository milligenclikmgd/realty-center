...
function HomePage({ counts, currentSlide, selectedCity, setSelectedCity, openDrawer }: any) {
  ...
  return (
    <div className="home-page-static">
      ...
      <section className="gold-showcase relative isolate overflow-hidden border-y border-sky-200 py-7"><div className="relative z-10 mx-auto max-w-7xl px-5 lg:px-8"><h2 className="mb-4 text-center text-xl font-black tracking-tight text-slate-950 sm:text-2xl">REALTY CENTER <span className="text-sky-600">YILDIZLARI</span></h2><div className="grid gap-5 xl:grid-cols-2 xl:gap-7"><div className="grid grid-cols-3 gap-3">{SAMPLE_OFFICES.slice(0, 3).map((office) => <Link key={office.id} to="/ofislerimiz" className="gold-showcase-card group overflow-hidden rounded-xl border border-sky-200/90 bg-white p-1.5 transition hover:-translate-y-1 hover:border-sky-400"><img src={office.image} alt={office.name} className="h-24 w-full rounded-lg object-cover"/><h3 className="mt-2 px-1 line-clamp-1 text-xs font-black text-slate-950">{office.name}</h3><p className="mb-1 px-1 text-[10px] text-slate-600">{office.district}, {office.city}</p></Link>)}</div><div className="grid grid-cols-3 gap-3">{SAMPLE_AGENTS.slice(0, 3).map((agent) => <Link key={agent.id} to="/danismanlarimiz" className="gold-showcase-card group overflow-hidden rounded-xl border border-sky-200/90 bg-white p-1.5 text-center transition hover:-translate-y-1 hover:border-sky-400"><img src={agent.image} alt={agent.name} className="mx-auto h-24 w-24 rounded-full object-cover ring-1 ring-sky-200"/><h3 className="mt-2 line-clamp-1 text-xs font-black text-slate-950">{agent.name}</h3><p className="mb-1 line-clamp-1 text-[10px] text-slate-600">{agent.title}</p></Link>)}</div></div></div></section>
