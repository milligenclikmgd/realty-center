...
function WorldRealtyCenterModule() {
  ...
  return <section className="relative overflow-hidden bg-[#06182f] py-14 text-white sm:py-20"><div className="pointer-events-none absolute -left-32 top-0 h-80 w-80 rounded-full bg-red-600/20 blur-3xl"/><div className="pointer-events-none absolute -right-20 bottom-0 h-96 w-96 rounded-full bg-blue-500/15 blur-3xl"/><div className="relative mx-auto max-w-7xl px-6 lg:px-12"><div className="grid items-stretch gap-6 lg:grid-cols-[.72fr_1.28fr]"><div className="flex flex-col justify-between rounded-[2rem] border border-white/10 bg-white/[.045] p-7 shadow-2xl backdrop-blur sm:p-9"><div><p className="flex items-center gap-2 text-[11px] font-black tracking-[.24em] text-red-300"><Globe className="h-4 w-4"/> KÜRESEL AĞ <span className="text-white/35">GLOBAL NETWORK</span></p><h2 className="mt-5 text-4xl font-black leading-[.95] tracking-tight sm:text-5xl">DÜNYADA <span className="text-red-500">REALTY CENTER®</span></h2><p className="mt-6 max-w-md text-sm leading-7 text-slate-300">Ülkelerden şehirlere uzanan büyüyen ağımızı keşfedin. Haritayı yakınlaştırdıkça yerel ofis noktaları belirir.</p></div><div className="mt-8 grid gap-3"><div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.055] p-4"><Globe className="h-5 w-5 text-red-400"/><div><p className="text-lg font-black">20+ ülke</p><p className="text-[11px] font-bold text-slate-400">Küresel erişim</p></div></div><div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.055] p-4"><Building2 className="h-5 w-5 text-red-400"/><div><p className="text-lg font-black">21 ofis noktası</p><p className="text-[11px] font-bold text-slate-400">Yerel uzmanlık</p></div></div></div></div><div className="overflow-hidden rounded-[2rem] border border-white/15 bg-white p-2 shadow-2xl sm:p-3"><div className="flex items-center justify-between rounded-t-[1.45rem] bg-[#0b2443] px-5 py-3 text-xs font-black"><span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_12px_#34d399]"/> CANLI KÜRESEL AĞ</span><span className="hidden text-[10px] tracking-[.14em] text-slate-400 sm:block">YAKINLAŞTIR · KEŞFET</span></div><div className="relative h-[400px] overflow-hidden rounded-b-[1.45rem] bg-slate-100 sm:h-[520px]"><MapContainer center={[22,8]} zoom={2} minZoom={2} maxZoom={8} scrollWheelZoom className="h-full w-full"><TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/><WorldZoomListener />{offices.map((office,index) => <Marker key={`${office.country}-${office.city}`} position={office.pos} icon={markerIcon}><Tooltip permanent direction="top" offset={[0,-14]} opacity={1}>{zoom >= 4 ? office.city : offices.findIndex((item) => item.country === office.country) === index ? office.country : ''}</Tooltip><Popup><strong>{office.city}</strong><br/>{office.country}<br/><span className="text-xs text-red-700">REALTY CENTER® ağı</span></Popup></Marker>)}</MapContainer><div className="pointer-events-none absolute bottom-4 left-4 z-[500] flex items-center gap-2 rounded-xl bg-white/95 px-3 py-2 text-[11px] font-black text-slate-700 shadow-lg backdrop-blur"><MapPin className="h-3.5 w-3.5 text-red-700"/>{zoom < 4 ? 'Ülke görünümü' : 'Şehir ve ofis görünümü'}</div></div></div></div></div></section>;
}
