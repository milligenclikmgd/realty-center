  ...
  return (
    <header className="header-corporate-glass sticky top-0 isolate z-40 w-full border-b border-white/15 bg-gradient-to-r from-[#8B0015] via-[#B9001B] to-[#D90829] px-6 py-2.5 text-white shadow-lg shadow-black/30 transition-shadow duration-500">
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-6 text-sm font-extrabold xl:flex">{links.map(([label, to]) => <Link key={to} to={to} className={menuLinkClass(to)}>{label}</Link>)}<div className="group relative"><button className="relative py-2 text-slate-100 transition hover:text-white after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-white after:transition-all group-hover:after:w-full">Bilgi Merkezi ▾</button><div className="information-dropdown invisible absolute right-0 top-full z-50 mt-4 w-[540px] translate-y-3 rounded-2xl border border-slate-200 bg-white/98 p-3 opacity-0 shadow-[0_18px_45px_rgba(30,10,15,.24)] backdrop-blur-xl transition-all duration-300 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"><div className="mb-3 flex items-center gap-2 px-2 pt-1"><span className="h-2 w-2 rounded-full bg-[#CD011E] shadow-[0_0_10px_rgba(205,1,30,.65)]"/><p className="text-[10px] font-black tracking-[.18em] text-[#CD011E]">BİLGİ MERKEZİ</p></div><div className="grid grid-cols-[.95fr_1.05fr] gap-3"><Link to="/#akademi" className="group/cover relative min-h-52 overflow-hidden rounded-xl bg-slate-900 p-5"><img src="/slider/slider3.jpg" alt="" className="absolute inset-0 h-full w-full object-cover opacity-80 transition duration-500 group-hover/cover:scale-110"/><div className="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/30 to-transparent"/><div className="relative flex h-full flex-col justify-end"><span className="w-fit rounded-full bg-[#CD011E] px-2 py-1 text-[9px] font-black text-white">ÖNE ÇIKAN</span><p className="mt-2 text-sm font-black leading-tight text-white">Ev Alırken Tapu Bilgisi Nasıl Kontrol Edilir?</p><span className="mt-2 text-[10px] font-bold text-white/75">Rehberi İncele →</span></div></Link><div className="space-y-1">{[['Gayrimenkul Rehberi','Ev alma, satma ve kiralama rehberleri'],['Gayrimenkul Hukuku','Tapu, sözleşme ve hukuki bilgiler'],['Sektörden Haberler','Piyasa ve sektör gündemi'],['Yatırım Analizleri','Bölge ve yatırım fırsatları']].map(([title,description])=><Link key={title} to="/#akademi" className="information-dropdown-item group/item block rounded-xl px-3 py-3 transition hover:bg-[#fff1f3]"><span className="block text-xs font-black text-slate-900 transition group-hover/item:text-[#CD011E]">{title}</span><span className="mt-1 block text-[10px] font-medium leading-relaxed text-slate-500">{description}</span></Link>)}</div></div></div></div></nav>
      ...
      {mobileOpen && <nav className="relative z-10 xl:hidden mt-3 grid grid-cols-2 gap-2 border-t border-white/10 pt-3 pb-2 text-sm font-black text-white">{links.map(([label, to]) => <Link key={to} onClick={close} to={to} className="rounded-xl bg-white/5 px-3 py-3 transition hover:bg-white/10">{label}</Link>)}<div className="col-span-2 rounded-xl bg-white/10 p-3"><p className="text-xs font-black text-white">Bilgi Merkezi</p><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] font-bold text-white/75"><Link onClick={close} to="/#akademi">Gayrimenkul Rehberi</Link><Link onClick={close} to="/#akademi">Gayrimenkul Hukuku</Link><Link onClick={close} to="/#akademi">Sektörden Haberler</Link><Link onClick={close} to="/#akademi">Yatırım Analizleri</Link></div></div><Link onClick={close} to="/panel" className="col-span-2 rounded-xl border border-[#ef6577] bg-[#CD011E] px-3 py-3 text-center transition hover:brightness-110">Panele Git</Link></nav>}
    </header>
  );
}
