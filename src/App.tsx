...
function Header({ language, setLanguage }: { language: StaticLanguage; setLanguage: (language: StaticLanguage) => void }) {
  ...
  return (
    <header className="header-corporate-glass sticky top-0 isolate z-40 w-full border-b border-white/15 bg-gradient-to-r from-[#8B0015] via-[#B9001B] to-[#D90829] px-6 py-2.5 text-white shadow-lg shadow-black/30 transition-shadow duration-500">
      ...
      {mobileOpen && <nav className="relative z-10 xl:hidden mt-3 grid grid-cols-2 gap-2 border-t border-white/10 pt-3 pb-2 text-sm font-black text-white">{links.map(([label, to]) => <Link key={to} onClick={close} to={to} className="rounded-xl bg-white/5 px-3 py-3 transition hover:bg-white/10">{label}</Link>)}<div className="col-span-2 rounded-xl bg-white/10 p-3"><p className="text-xs font-black text-white">BLOG</p><div className="mt-2 grid grid-cols-2 gap-2 text-[11px] font-bold text-white/75"><Link onClick={close} to="/#akademi">Rehber</Link><Link onClick={close} to="/#akademi">Hukuk</Link><Link onClick={close} to="/#akademi">Haberler</Link><Link onClick={close} to="/#akademi">Analizler</Link></div></div><Link onClick={close} to="/panel" className="col-span-2 rounded-xl border border-[#ef6577] bg-[#CD011E] px-3 py-3 text-center transition hover:brightness-110">Panele Git</Link></nav>}
    </header>
  );
}
