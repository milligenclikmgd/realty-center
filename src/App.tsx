...
function Header({ scrolled }: { scrolled: boolean }) {
  ...
  return (
    <header className={`sticky top-0 z-40 w-full border-b border-white/10 text-white transition-all duration-500 ${scrolled ? 'bg-[#0a1224]/95 py-1.5 px-5 lg:px-10 shadow-xl shadow-slate-950/40 backdrop-blur-xl' : 'bg-[#111b31]/96 py-2.5 px-6 lg:px-12 shadow-lg shadow-slate-950/25 backdrop-blur-md'}`}>
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[3px] bg-[#CD011E]" />
      <div className="flex items-center justify-between gap-4">
        <Link to="/" onClick={close} className={`relative -my-2 flex items-center rounded-r-2xl bg-[#CD011E] px-4 py-2 shadow-lg shadow-black/20 transition-transform hover:scale-[1.02] ${scrolled ? 'lg:-my-1.5' : ''}`}><img src="/rlogo.png" alt="Realty Center" className={`w-auto object-contain brightness-0 invert transition-all duration-500 ${scrolled ? 'h-11 lg:h-12' : 'h-12 lg:h-14'}`} /></Link>
        <nav className="hidden xl:flex items-center gap-6 text-sm font-extrabold text-slate-100">{links.map(([label, to]) => <Link key={to} to={to} className="relative py-2 transition hover:text-white after:absolute after:bottom-0 after:left-1/2 after:h-0.5 after:w-0 after:-translate-x-1/2 after:bg-[#CD011E] after:transition-all hover:after:w-full">{label}</Link>)}<Link to="/danisman-basvuru" className="relative py-2 font-black text-slate-100 transition hover:text-white after:absolute after:bottom-0 after:left-1/2 after:h-0.5 after:w-0 after:-translate-x-1/2 after:bg-[#CD011E] after:transition-all hover:after:w-full">Danışman Ol</Link><Link to="/franchise-basvuru" className="relative py-2 font-black text-slate-100 transition hover:text-white after:absolute after:bottom-0 after:left-1/2 after:h-0.5 after:w-0 after:-translate-x-1/2 after:bg-[#CD011E] after:transition-all hover:after:w-full">Franchise Ol!</Link></nav>
      ...
      {mobileOpen && <nav className="xl:hidden mt-3 grid grid-cols-2 gap-2 border-t border-white/10 pt-3 pb-2 text-sm font-black text-white">{links.map(([label, to]) => <Link key={to} onClick={close} to={to} className="rounded-xl bg-white/5 px-3 py-3 transition hover:bg-white/10">{label}</Link>)}<Link onClick={close} to="/danisman-basvuru" className="rounded-xl bg-white/5 px-3 py-3 text-left transition hover:bg-white/10">Danışman Ol</Link><Link onClick={close} to="/franchise-basvuru" className="rounded-xl bg-[#CD011E] px-3 py-3 text-left text-white transition hover:brightness-110">Franchise Ol!</Link><Link onClick={close} to="/panel" className="col-span-2 rounded-xl border border-[#ef6577] bg-[#CD011E] px-3 py-3 text-center transition hover:brightness-110">Panele Git</Link></nav>}
    </header>
  );
}
