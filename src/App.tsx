...
function TurkeyListingMap() {
    ...
    fetch('/turkey.svg')
      ...
      .then((svgText) => {
        ...
        parsed.querySelectorAll<SVGGElement>('g[data-city-name]').forEach((group) => {
          ...
          group.querySelectorAll('path').forEach((path) => path.setAttribute('style', 'fill:#f8c8d0;stroke:#c63952;stroke-width:0.75;transition:fill 180ms ease;'));
        });
  ...
  const handleMapMove = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (activeGroupRef.current !== group) {
      paintGroup(activeGroupRef.current, '#f8c8d0');
      paintGroup(group, '#a30b1d');
  ...
  return (
    <header className={`sticky top-0 z-40 w-full border-b border-white/25 text-white transition-all duration-500 ${scrolled ? 'bg-[#a90818]/95 py-1.5 px-5 lg:px-10 shadow-lg shadow-black/30 backdrop-blur-md' : 'bg-[#c90a1b]/92 py-2.5 px-6 lg:px-12 shadow-md shadow-black/20 backdrop-blur-sm'}`}>
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2"><Link to="/panel" className="group relative hidden overflow-hidden rounded-xl border border-white/70 bg-white/95 px-5 py-2 text-sm font-black text-[#a30b1d] shadow-lg shadow-black/20 transition hover:-translate-y-0.5 hover:border-white hover:shadow-xl sm:inline-flex"><span className="relative z-10">Panel</span><span className="absolute inset-y-0 -left-1/2 w-1/3 -skew-x-12 bg-gradient-to-r from-transparent via-white to-transparent opacity-0 transition-all duration-700 group-hover:left-[130%] group-hover:opacity-90"/></Link><button onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menüyü aç" className="xl:hidden inline-flex h-11 w-11 items-center justify-center rounded-lg border border-white/40 text-white transition hover:border-white hover:bg-white/10">{mobileOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button></div>
      </div>
