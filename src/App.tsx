  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{groups.map((group,index) => <section key={group.title} className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-xl"><div className="relative h-24 overflow-hidden"><img src={group.items[0][2]} alt="" className="h-full w-full object-cover opacity-60 transition duration-700 group-hover:scale-105"/><div className="absolute inset-0" style={{background:`linear-gradient(90deg, ${group.color}f4 0%, ${group.color}dc 62%, ${group.color}a8 100%)`}}/><div className="absolute inset-x-4 bottom-3 flex items-end justify-between"><div><p className="text-[9px] font-black tracking-[.18em] text-white/80">{group.label}</p><h2 className="mt-0.5 text-base font-black text-white">{group.title}</h2></div><span className="text-[10px] font-bold text-white/75">0{index+1}</span></div></div><div className="p-3.5"><p className="min-h-8 text-[11px] leading-4 text-slate-500">{group.description}</p><div className="mt-3 grid grid-cols-2 gap-2">{group.items.map(([label,propertyType,image],itemIndex) => <button key={`${group.title}-${label}`} onClick={() => openCategory(group, propertyType)} className="group/item relative aspect-square overflow-hidden rounded-lg text-left shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-lg"><img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-55 transition duration-500 group-hover/item:scale-110"/><span className="absolute inset-0 transition" style={{background:`linear-gradient(145deg, ${group.color}f0 0%, ${group.color}d8 58%, ${group.color}b8 100%)`}}/><span className="relative flex h-full flex-col justify-between p-2.5"><span className="text-[8px] font-black tracking-[.14em] text-white/75">0{itemIndex+1}</span><span><span className="block text-xs font-black leading-4 text-white">{label}</span><span className="mt-0.5 flex items-center gap-1 text-[8px] font-bold uppercase tracking-wide text-white/85">{propertyType || 'Tüm seçenekler'} <ArrowRight className="h-3 w-3 transition group-hover/item:translate-x-1"/></span></span></span></button>)}</div></div></section>)}</div>
      </div>
    </div>
  );
}
