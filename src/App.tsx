...
function TurkeyListingMap() {
  ...
  const handleMapOut = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const enteringGroup = nextTarget?.closest('g[data-realty-city]') as SVGGElement | null;
    if (leavingGroup && leavingGroup === activeGroupRef.current && leavingGroup !== enteringGroup) clearActiveGroup();
  };

  const handleMapLeave = () => clearActiveGroup();
...
function Header({ scrolled }: { scrolled: boolean }) {
  ...
  return (
    <header className={`sticky top-0 z-40 w-full border-b border-white/25 text-white transition-all duration-500 ${scrolled ? 'bg-[#CD011E]/95 py-1.5 px-5 lg:px-10 shadow-lg shadow-black/30 backdrop-blur-md' : 'bg-[#CD011E]/92 py-2.5 px-6 lg:px-12 shadow-md shadow-black/20 backdrop-blur-sm'}`}>
