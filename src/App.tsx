...
function RealtyNetworkActivityPanel() {
  ...
  return <><button type="button" onClick={() => setWorldOpen(true)} className="relative min-h-[238px] w-full overflow-hidden rounded-2xl border border-slate-200 bg-white text-left shadow-lg transition hover:border-red-300">
    ...
    <div className="relative z-10 p-5"><p className="text-[10px] font-black tracking-[.22em] text-red-700">DÜNYADA REALTY CENTER®</p><h3 className="mt-2 text-xl font-black text-slate-950">Küresel bağlantı ağı</h3>
...
function BarterBankModuleV3() {
  ...
  const [sent, setSent] = useState(false);
  const fields = (type: typeof offer) => type === 'Ev' ? ['İl', 'İlçe', 'Mahalle', 'Konut özellikleri', 'Fiyat'] : type === 'Arsa' ? ['İl', 'İlçe', 'Mahalle', 'Ada', 'Parsel', 'Fiyat'] : type === 'Otomobil' ? ['Marka', 'Model', 'Fiyat'] : ['Şehir / İlçe', 'Açıklama', 'Fiyat'];
  const column = (title: string, type: typeof offer, setType: (type: typeof offer) => void, prefix: string) => <section className="border border-slate-200 bg-slate-50 p-4 sm:p-5"><h3 className="text-lg font-black text-slate-900">{title}</h3><p className="mt-1 text-xs text-slate-500">{title === 'TAKAS ÖNERİSİ' ? 'Vermek istediğiniz değer' : 'Almak istediğiniz değer'}</p><label className="mt-4 block text-[10px] font-black tracking-widest text-slate-600">DEĞER TÜRÜ<select value={type} onChange={(e) => setType(e.target.value as typeof offer)} className="mt-2 w-full border border-slate-200 bg-white p-3 text-sm"><option>Ev</option><option>Arsa</option><option>Otomobil</option><option>Diğer</option></select></label><div className="mt-3 grid gap-3">{fields(type).map((field) => <input key={field} required name={`${prefix}-${field}`} placeholder={field} className="border border-slate-200 bg-white px-3 py-3 text-sm"/>)}</div></section>;
  ...
  if (sent) return <section id="barter-bank" className="bg-slate-50 py-14"><div className="mx-auto max-w-5xl px-6"><div className="bg-[#CD011E] p-10 text-center text-white"><h2 className="text-2xl font-black">Barter kaydınız alındı.</h2><p className="mt-3 text-sm text-white/85">Teklifiniz ve talebiniz yönetici paneline iletildi.</p></div></div></section>;
  return <section id="barter-bank" className="border-b border-slate-200 bg-slate-50 py-14"><div className="mx-auto max-w-7xl px-6 lg:px-12"><div className="overflow-hidden border border-slate-200 bg-white shadow-xl"><header className="bg-[#CD011E] px-7 py-8 text-white sm:px-10"><p className="text-3xl font-black tracking-[.14em] sm:text-5xl">BARTER BANK</p><p className="mt-3 text-sm text-white/85">Fazlanı ver, eksiğini al. Tek formda hem takas önerinizi hem takas talebinizi bırakın.</p></header><form onSubmit={submit} className="p-6 sm:p-10"><div className="grid gap-5 lg:grid-cols-2">{column('TAKAS ÖNERİSİ', offer, setOffer, 'öneri')}{column('TAKAS TALEBİ', request, setRequest, 'talep')}</div><div className="mt-5 grid gap-4 sm:grid-cols-2"><select required name="Barter türü" className="border border-slate-200 px-4 py-3 text-sm"><option value="">Barter türünü seçin</option><option>Tam Barter</option><option>Kısmi Barter</option></select><input required name="Telefon" placeholder="Telefon numaranız" className="border border-slate-200 px-4 py-3 text-sm"/></div><label className="mt-5 flex items-start gap-2 text-[11px] leading-5 text-slate-500"><input required type="checkbox" className="mt-1 accent-red-700"/>KVKK ve <Link to="/kullanim-kosullari" className="font-black text-red-700">Site Kullanım Politikası</Link> metinlerini okudum ve onaylıyorum.</label><button className="mt-6 w-full bg-[#CD011E] px-5 py-4 text-sm font-black text-white">TAKAS ÖNERİSİ VE TALEBİNİ GÖNDER</button></form></div></div></section>;
}

function ListingActionShortcuts() {
  return <aside className="grid self-start gap-2 sm:grid-cols-2"><Link to="/#barter-bank" className="border border-red-700 bg-[#CD011E] px-4 py-3 text-white shadow-sm transition hover:bg-red-800"><p className="text-[10px] font-black tracking-widest text-white/85">BARTER BANK</p><strong className="mt-1 block text-sm leading-tight text-white">Takas fırsatlarını görün →</strong></Link><Link to="/#buyer-request" className="border border-red-700 bg-[#CD011E] px-4 py-3 text-white shadow-sm transition hover:bg-red-800"><p className="text-[10px] font-black tracking-widest text-white/85">SİZ ARADIĞINIZI SÖYLEYİN</p><strong className="mt-1 block text-sm leading-tight text-white">Size uygun ilanı bulalım →</strong></Link></aside>;
}
