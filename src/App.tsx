...
function ListingCategoriesPage() {
  const navigate = useNavigate();
  const groups = [
    { title: 'Satılık Gayrimenkuller', label: 'SATILIK', transaction: 'Satılık', icon: Home, accent: '#CD011E', items: [['Konut','Ev','https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=80&w=900'],['Ticari','Ticari Gayrimenkul','https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80&w=900'],['Arsa','Arsa','https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&q=80&w=900']] },
    { title: 'Kiralık Gayrimenkuller', label: 'KİRALIK', transaction: 'Kiralık', icon: Key, accent: '#2779d6', items: [['Konut','Ev','https://images.unsplash.com/photo-1600566753086-00f18fb6b3ea?auto=format&fit=crop&q=80&w=900'],['İşyeri','Dükkan','https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80&w=900'],['Villa','Villa','https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&q=80&w=900']] },
    ...
    { title: 'Yeni Projeler', label: 'PROJELER', route: '/projelerimiz', icon: Building2, accent: '#123499', items: [['Yeni Konut','','https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&q=80&w=900'],['Ticari Proje','','https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=900'],['Villa Projesi','','https://images.unsplash.com/photo-1600047509807-ba8f99d2cdde?auto=format&fit=crop&q=80&w=900']] },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{groups.map((group,index) => { const Icon = group.icon; const allLabel = group.route ? 'Tüm projeleri gör' : `Tüm ${group.label.toLocaleLowerCase('tr-TR')} ilanları gör`; return <section key={group.title} className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,.07)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(15,23,42,.12)]"><div className="mb-4 flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: `${group.accent}12`, color: group.accent }}><Icon className="h-5 w-5" strokeWidth={2.2}/></div><div className="min-w-0 flex-1"><p className="text-[10px] font-black tracking-[.16em]" style={{ color: group.accent }}>{group.label}</p><h2 className="mt-0.5 truncate text-[15px] font-black tracking-tight text-slate-900">{group.title}</h2></div><span className="text-sm font-black text-slate-300">0{index+1}</span></div><div className="grid grid-cols-2 gap-3">{group.items.map(([label,propertyType,image]) => <button key={`${group.title}-${label}`} onClick={() => openCategory(group, propertyType)} className="group/item relative aspect-[.92] overflow-hidden rounded-xl bg-slate-900 text-left"><img src={image} alt={label} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover/item:scale-105"/><span className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-900/10 to-transparent"/><span className="absolute inset-x-3 bottom-3 flex items-end justify-between gap-2"><span className="text-sm font-black leading-tight text-white">{label}</span><ArrowRight className="h-4 w-4 shrink-0 text-white transition group-hover/item:translate-x-1"/></span></button>)}<button onClick={() => openCategory(group, '')} className="group/all flex aspect-[.92] flex-col items-start justify-between rounded-xl p-3 text-left transition hover:brightness-110" style={{ backgroundColor: group.accent }}><span className="text-[10px] font-black tracking-[.12em] text-white/65">TÜMÜNÜ GÖR</span><span className="flex w-full items-end justify-between gap-2 text-sm font-black leading-tight text-white"><span>{allLabel}</span><ArrowRight className="h-4 w-4 shrink-0 transition group-hover/all:translate-x-1"/></span></button></div></section>})}</div>
      </div>
    </div>
  );
}
