  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{groups.map((group) => <section key={group.title}><div className={`overflow-hidden rounded-2xl border-2 bg-white shadow-[0_10px_24px_rgba(15,23,42,.07)] ${group.tone}`}><div className={`flex items-center gap-2.5 px-4 py-3.5 text-white ${group.head}`}><span className="text-2xl">{group.icon}</span><div><h2 className="text-base font-black tracking-[.08em]">{group.title}</h2><p className="text-[9px] font-bold text-white/75">Alt kategoriyi seçin</p></div></div><div className="grid aspect-[4/3] grid-cols-2 grid-rows-2 divide-x divide-y divide-slate-200">{group.items.map(([label,propertyType,image]) => <button key={`${group.title}-${label}`} onClick={() => openCategory(group, propertyType)} className={`group relative flex flex-col items-center justify-center gap-1.5 overflow-hidden p-3 text-center text-xs font-black text-slate-800 transition ${group.cell}`}><img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-[.16] grayscale-[20%] transition duration-300 group-hover:scale-110 group-hover:opacity-[.28]"/><span className="absolute inset-0 bg-white/55 transition group-hover:bg-white/35"/><span className="relative drop-shadow-sm">{label}</span><ArrowRight className="relative h-3.5 w-3.5 text-slate-400 transition group-hover:translate-x-1 group-hover:text-current"/></button>)}</div></div></section>)}</div>
      </div>
    </div>
  );
}
