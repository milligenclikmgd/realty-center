    ...
    <div className="mx-auto max-w-7xl px-5 lg:px-8">
      <div className="ai-assistant-hero relative overflow-hidden rounded-3xl px-6 py-10 text-white shadow-2xl sm:px-10">
        ...
        <div className="mt-6 inline-flex rounded-xl border border-white/15 bg-[#03142e]/60 p-1">
          <button onClick={() => setAiTab('assistant')} className={`rounded-lg px-4 py-2 text-xs font-black transition ${aiTab === 'assistant' ? 'bg-cyan-500 text-slate-950 shadow' : 'text-cyan-100/70 hover:text-white'}`}>🤖 Akıllı Arama</button>
          <button onClick={() => setAiTab('valuation')} className={`rounded-lg px-4 py-2 text-xs font-black transition ${aiTab === 'valuation' ? 'bg-cyan-500 text-slate-950 shadow' : 'text-cyan-100/70 hover:text-white'}`}>⌁ Otomatik Değerleme</button>
        </div>
        ...
        </div><p className="mt-3 text-xs text-slate-400">Bu ilk sürüm, ücretsiz demo amaçlı olarak site verisi ve örnek bölge metrikleriyle çalışır.</p></> : <div className="mt-5 rounded-2xl bg-white/10 p-4 backdrop-blur-sm"><div className="grid gap-3 sm:grid-cols-3"><input value={valuation.city} onChange={e=>setValuation({...valuation,city:e.target.value})} className="rounded-xl border border-white/20 bg-white/95 px-3 py-2.5 text-sm font-bold text-slate-800" placeholder="İl"/><input value={valuation.district} onChange={e=>setValuation({...valuation,district:e.target.value})} className="rounded-xl border border-white/20 bg-white/95 px-3 py-2.5 text-sm font-bold text-slate-800" placeholder="İlçe"/><select value={valuation.propertyType} onChange={e=>setValuation({...valuation,propertyType:e.target.value})} className="rounded-xl bg-white px-3 py-2.5 text-sm font-bold text-slate-800"><option>Daire</option><option>Villa</option><option>Ofis</option><option>Arsa</option></select><input value={valuation.area} onChange={e=>setValuation({...valuation,area:e.target.value})} className="rounded-xl bg-white px-3 py-2.5 text-sm font-bold text-slate-800" placeholder="Brüt m²"/><select value={valuation.rooms} onChange={e=>setValuation({...valuation,rooms:e.target.value})} className="rounded-xl bg-white px-3 py-2.5 text-sm font-bold text-slate-800"><option>3+1</option><option>2+1</option><option>4+1</option></select><select value={valuation.age} onChange={e=>setValuation({...valuation,age:e.target.value})} className="rounded-xl bg-white px-3 py-2.5 text-sm font-bold text-slate-800"><option>0-4</option><option>5-10</option><option>11-20</option><option>20+</option></select></div><button onClick={()=>setValuationReady(true)} className="mt-3 rounded-xl bg-cyan-500 px-5 py-3 text-sm font-black text-slate-950 shadow transition hover:bg-cyan-300">Otomatik Değerle</button></div>}
      </div>
