  ...
  return (
    <header className="header-corporate-glass sticky top-0 isolate z-40 w-full border-b border-white/15 bg-gradient-to-r from-[#8B0015] via-[#B9001B] to-[#D90829] px-6 py-2.5 text-white shadow-lg shadow-black/30 transition-shadow duration-500">
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-6 text-sm font-extrabold xl:flex">{links.map(([label, to]) => <Link key={to} to={to} className={menuLinkClass(to)}>{label}</Link>)}<div className="group relative"><button className="relative py-2 text-slate-100 transition hover:text-white after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-white after:transition-all group-hover:after:w-full">Bilgi Merkezi ▾</button><div className="invisible absolute right-0 top-full z-50 mt-3 w-64 translate-y-2 rounded-2xl border border-white/15 bg-[#8B0015]/98 p-2 opacity-0 shadow-2xl backdrop-blur-md transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"><p className="px-3 pb-2 pt-1 text-[10px] font-black tracking-[.16em] text-white/60">BİLGİ MERKEZİ</p>{[['Gayrimenkul Rehberi','Ev alma, satma ve kiralama rehberleri'],['Gayrimenkul Hukuku','Tapu, sözleşme ve hukuki bilgiler'],['Sektörden Haberler','Piyasa ve sektör gündemi'],['Yatırım Analizleri','Bölge ve yatırım fırsatları']].map(([title,description])=><Link key={title} to="/#akademi" className="block rounded-xl px-3 py-2.5 transition hover:bg-white/10"><span className="block text-xs font-black text-white">{title}</span><span className="mt-0.5 block text-[10px] font-medium text-white/65">{description}</span></Link>)}</div></div></nav>
