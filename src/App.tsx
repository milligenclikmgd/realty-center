  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        ...
        <div className="mt-9 grid gap-6 lg:grid-cols-2">{groups.map((group,index) => <section key={group.title} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:border-slate-300 hover:shadow-xl"><div className="flex items-start justify-between border-b border-slate-100 px-5 py-4"><div><p className="text-[10px] font-black tracking-[.18em] text-red-700">{group.label}</p><h2 className="mt-1 text-xl font-black tracking-tight text-slate-900">{group.title}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{group.description}</p></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-[10px] font-black text-slate-500">0{index+1}</span></div><div className="grid grid-cols-2 gap-px bg-slate-200">{group.items.map(([label,propertyType,image],itemIndex) => <button key={`${group.title}-${label}`} onClick={() => openCategory(group, propertyType)} className="group/item relative aspect-[4/3] overflow-hidden bg-slate-900 text-left"><img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-75 transition duration-500 group-hover/item:scale-105"/><span className="absolute inset-0 bg-gradient-to-t from-slate-950/85 via-slate-900/25 to-transparent"/><span className="absolute left-4 top-3 text-[10px] font-black tracking-[.14em] text-white/70">0{itemIndex+1}</span><span className="absolute inset-x-4 bottom-3"><span className="block text-base font-black leading-tight text-white">{label}</span><span className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-white/75">{propertyType || 'Tüm seçenekler'} <ArrowRight className="h-3 w-3 transition group-hover/item:translate-x-1"/></span></span></button>)}</div></section>)}</div>
      </div>
    </div>
  );
}
